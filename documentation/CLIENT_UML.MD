...
classDiagram
    ...
    class ClientManager {
        ...
        -getClient(string username) shared_ptr~Client~
        -getChatting() shared_ptr~Client~
        -reset() void
        -disconnect() void
        -startAudioSocket() void
        -stopAudioSocket() void

        +shared_ptr~SceneManager~ sceneManager
        +shared_ptr~Audio> audioDevice
        +vector~shared_ptr~Client~~ clients
        +shared_ptr~EventManager~ eventManager
        +shared_ptr~PacketManager~ packetManager
        +shared_ptr~Client~ self
        +shared_ptr~Transporter~ transporter
        +shared_ptr~AudioSettings~ audioSettings
        +shared_ptr~AudioSocket~ audioSocket
        -thread *_sendAudioThread
        -thread *_receiveAudioThread
        -_sendAudio() void
        -_receiveAudio() void
    }
    ...
    class Socket {
        ...
        +setEventManager(shared_ptr~EventManager~ eventManager) void
    }
    ...
    class QtSocket {
        ...
        -~unique_ptr<QTcpSocket>~ _socket
        -void (EventManager::* _readHandler)(std::string message)
    }
    ...
    class Audio {
        +getAudioCodec() shared_ptr~AudioCodec~
        +capture() string
        +write(DecodedAudio &audio) void
    }

    PortAudio <|-- Audio

    class PortAudio {
        -shared_ptr~AudioCodec~ _audioCodec
    ...
    class AudioCodec {
        +encode(float *buffer) string
        +decode(string buffer) DecodedAudio
    }

    OpusCodec <|-- AudioCodec
    ...
    class AudioSettings {
        +setReceiver(AudioReceiver receiver) void
        +getReceiver() AudioReceiver
        +setRunning(bool isRunning) void
        +isRunning() bool
        -mutex _mutex
    ...
    class AudioSocket {
        +sendAudio(string audio, string hostname, unsigned int port) void
        +receiveAudio(AudioSettings &settings, Audio &audio) void
        +getPort() unsigned int
    }

    QtAudioSocket <|-- AudioSocket
